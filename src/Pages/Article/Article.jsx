...
const Article = () => {
    ...
    const handleComment = () => {
        axios.post(API_URL + '/comment/' + getUserId() + '/' + id, {
            ...
            .then((res) => {
                console.log(res)
                setComments([...comments, { id: comments[comments ? comments.length - 1: 0].id + 1, username: getCurrentUser().user.username, comment_text: commentValue }])
            }).catch((err) => console.log(err))
    }
